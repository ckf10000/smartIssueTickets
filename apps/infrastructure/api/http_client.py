...
class HttpService(object):
    ...
    def __parse_data_response(self, response: requests.Response) -> t.Dict:
        ...
        if 'application/json' in content_type or 'text/json' in content_type:
            ...
            # 纯文本类型
            data = dict(code=response.status_code, message=get_html_title(html=response.text), data=response.text)
        else:
            # 其他类型，默认视为二进制内容
            content = response.content.decode('utf-8')
            data = dict(code=response.status_code, message=get_html_title(html=content), data=content)
